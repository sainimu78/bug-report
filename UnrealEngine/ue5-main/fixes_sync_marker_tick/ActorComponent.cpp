...
static void CompileAnimBlueprint(const FString& TargetName)
{
	...
	for (const FAssetData& AssetData : BlueprintList)
	{
		...
		{
			if (UBlueprint* BlueprintObj = Cast<UBlueprint>(AssetData.GetAsset()))
			{
				...
				FKismetEditorUtilities::CompileBlueprint(BlueprintObj, CompileOptions, NULL);
				break;
			}
		}
	}
}
static void CompileAnimBlueprint(const UClass* cls)
{
	CompileAnimBlueprint(FBlueprintEditorUtils::GetClassNameWithoutSuffix(cls));
}

static float g_testElapsedTime = 0.0f;
static void TestFrameTick(bool isTestLoopBegin, const uint32 testFramesCount, const float testStartFrameDeltaTime, const uint32 testStartFrame, const uint32 testFinishFrame, const uint32 testEndFrame, uint32& testFrame, float& testDeltaTime)
{
	auto testElapsedTime = g_testElapsedTime;
	if (isTestLoopBegin)
	{
		testFrame = (testFrame % testFramesCount) + 1;
	}
	if (testFrame == testStartFrame)
	{
		testDeltaTime = testStartFrameDeltaTime;
		g_testElapsedTime = testDeltaTime;
	}
	else if (testFrame == testFinishFrame)
	{
		testDeltaTime = g_testElapsedTime;
	}
	else
	{
		if (testFrame != testEndFrame)
		{
			if (isTestLoopBegin)
				g_testElapsedTime += testDeltaTime;
		}
...
static void TestSetupForTickAndReanderSync()
{
	//begin, 使Game Tick(World, Game object, Component之类的Tick)与渲染结果完全同步(并非只与RenderThread同步, 还需要与RHI同步)
	//如此设置的原因是默认设置, VSync(false), GTSyncType(0), OneFrameThreadLag(true), 使渲染滞后2帧
	//现象如testFrame为3时, 场景中看到的画面为第1帧该有的画面
	//注意这些变量为全局, 如果影响其它功能, 需要将这些变量改为原值, 如通过命令修改
	//该方法还在验证阶段, 一般地, 可通过观察画面或Log确定是否与Tick同步
	//
	//启用VSync
	static auto CVarVsync = IConsoleManager::Get().FindConsoleVariable(TEXT("r.VSync"));
	if (CVarVsync != NULL)
		CVarVsync->Set(true);
	//GameThread Sync方式, 1为与RHI同步, 即GameThread将等待RenderThread的特定CompletionEvent
	static auto SyncIntervalCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.GTSyncType"));
	if (SyncIntervalCVar != NULL)
		SyncIntervalCVar->Set(1);
	//禁用允许滞后1帧
	static auto CVarAllowOneFrameThreadLag = IConsoleManager::Get().FindConsoleVariable(TEXT("r.OneFrameThreadLag"));
	if (CVarAllowOneFrameThreadLag != NULL)
		CVarAllowOneFrameThreadLag->Set(false);
	//end
}

static bool TestHasProperty(const UObject* obj, const FName& propertyName)
{
	if (auto property = obj->GetClass()->FindPropertyByName(propertyName))
		return true;
	return false;
}

static void TestSetPropertyValueDouble(UObject* obj, const FName& propertyName, double value)
{
	if (auto property = obj->GetClass()->FindPropertyByName(propertyName))
	{
		if (const FDoubleProperty* casted = CastField<const FDoubleProperty>(property))
			casted->SetPropertyValue_InContainer(obj, value);
	}
}
static double TestGetPropertyValueDouble(const UObject* obj, const FName& propertyName)
{
	if (auto property = obj->GetClass()->FindPropertyByName(propertyName))
	{
		if (const FDoubleProperty* casted = CastField<const FDoubleProperty>(property))
			return casted->GetPropertyValue_InContainer(obj);
	}
	return FDoubleProperty::GetDefaultPropertyValue();
}
static bool TestGetPropertyValueBool(const UObject* obj, const FName& propertyName)
{
	if (auto property = obj->GetClass()->FindPropertyByName(propertyName))
	{
		if (const FBoolProperty* casted = CastField<const FBoolProperty>(property))
			return casted->GetPropertyValue_InContainer(obj);
	}
	return FBoolProperty::GetDefaultPropertyValue();
}

//#define TEST_DISTANCE_MATCHING_BASED_STOP_ANIM_BLENDING

#ifdef TEST_DISTANCE_MATCHING_BASED_STOP_ANIM_BLENDING
static uint32 g_testFrame = 0;
static FILE* s_pf = NULL;
static float g_testDeltaTime = 0.0f;

static void TestBegin(UActorComponent* Target)
{
	auto owner = Target->GetOwner();

	const uint32 testSpecialFramesCount = 3;//start, finish, end
	const uint32 testFramesCount = 500;//测试帧的帧数, 如停步分100帧测试
	const uint32 testTotalFramesCount = testFramesCount + testSpecialFramesCount;//总帧数
	const uint32 testStartFrame = 1;//测试开始
	const uint32 testEndFrame = testTotalFramesCount;//测试结束和清理
	const uint32 testFinishFrame = testEndFrame - 1;//测试完成
	float testDeltaTime = 1.0f / testFramesCount;//seconds / frames, 其中1.0f为测试帧总时长, 如停步过程的时长(胶囊体开始停到不动)
	const float testStartFrameDeltaTime = 10.0f + 0.9f;//开始测试的时间, 如从跑开始停步; 10.0f表示一个足够大的时间用于跳过起跑, 0.9f为微调时间, 10.0f + 0.9f时刻姿势差不多为跑状态动作左脚与地面平行即将着地
	auto testFirstComponent = Cast<UCharacterMovementComponent>(Target);
	const bool isTestLoopBegin = testFirstComponent != nullptr;//按component的遍历顺序的首个component, isTestLoopBegin为true即一个测试循环开始, 如可用于逐帧调试设置断点

	TestSetupForTickAndReanderSync();
	TestFrameTick(isTestLoopBegin, testTotalFramesCount, testStartFrameDeltaTime, testStartFrame, testFinishFrame, testEndFrame, g_testFrame, testDeltaTime);
	g_testDeltaTime = testDeltaTime;

	if (auto movement = testFirstComponent)
	{
		if (g_testFrame == testStartFrame)
		{
			movement->AddInputVector(FVector(1.0f, 0.0f, 0.0f));
			s_pf = fopen("E:/Pose_UE.txt", "w");
		}
		else if (g_testFrame == testFinishFrame)
		{
			auto TM = owner->GetActorTransform();
			TM.SetTranslation(FVector(0.0f, 0.0f, 100.0f));
			owner->SetActorTransform(TM);
		}
	}
	else if (auto skelMesh = Cast<USkeletalMeshComponent>(Target))
	{
		double value = 0.0f;
		if (g_testFrame == testStartFrame)
			value = 0.1f;
		else if (g_testFrame == testStartFrame + 1)
			value = 0.2f;
		else if (g_testFrame == testStartFrame + 2)
			value = 0.3f;
		else
			value = 0.4f;

		//改AnimBlueprint变量
		auto animInstance = skelMesh->AnimScriptInstance;
		TestSetPropertyValueDouble(animInstance, TEXT("NewVar123"), value);

		skelMesh->SetDisablePostProcessBlueprint(true);//PostProcessAnimBlueprint可能对输出姿势有影响, 据观察输出, Lyra的ABP_Manny Mesh中设置的ABP_Manny_PostProcess对一些辅助骨骼的缩放有影响, 也许还有其它影响, 未确认
		animInstance->bUseMultiThreadedAnimationUpdate = false;//不启用异步更新, 避免节点回调获取到的数据滞后一帧, 不便调试

		if (g_testFrame == testEndFrame)
		{
			//CompileAnimBlueprint(TEXT("/Game/Characters/Heroes/Mannequin/Animations/AnimStarterPack/UE4ASP_HeroTPP_Start_Stop.UE4ASP_HeroTPP_Start_Stop"));
			//CompileAnimBlueprint(animInstance->GetClass());
			animInstance->InitializeAnimation();//只初始化, 没有重置节点(没有重创建节点)
			if (s_pf != NULL)
			{
				fclose(s_pf);
				s_pf = NULL;
...
static void TestEnd(UActorComponent* Target)
{
	auto owner = Target->GetOwner();

	if (auto movement = Cast<UCharacterMovementComponent>(Target))
	{
		//auto v0 = movement->Velocity;
		//auto v1 = movement->GetLastUpdateVelocity();
		//UE_LOG(LogActorComponent, Log, TEXT("X:%.6f, Y:%.6f, Z:%.6f; X:%.6f, Y:%.6f, Z:%.6f"), v0.X, v0.Y, v0.Z, v1.X, v1.Y, v1.Z);
		auto v0 = owner->GetActorLocation();
		UE_LOG(LogActorComponent, Log, TEXT("%u, x:%.6f, y:%.6f, z:%.6f"), g_testFrame, v0.X, v0.Y, v0.Z);
	}
	else if (auto skelMesh = Cast<USkeletalMeshComponent>(Target))
	{
		auto animInstance = skelMesh->AnimScriptInstance;
		auto hasVelocity = TestGetPropertyValueBool(animInstance, TEXT("HasVelocity"));
		auto hasAcceleration = TestGetPropertyValueBool(animInstance, TEXT("HasAcceleration"));
		UE_LOG(LogActorComponent, Log, TEXT("%u, HasVelocity: %s, HasAcceleration: %s"), g_testFrame, hasVelocity ? TEXT("true") : TEXT("false"), hasAcceleration ? TEXT("true") : TEXT("false"));

		if (s_pf != NULL)
		{
			auto& vec = skelMesh->GetComponentSpaceTransforms();
			fprintf(s_pf, "%u:\n", g_testFrame);

			auto foot_l = skelMesh->GetBoneIndex(TEXT("foot_l"));
			auto& it = vec[foot_l];
			const auto& T = it.GetTranslation();
			//fprintf(s_pf, "T: x:%.6f, y:%.6f, z:%.6f\n", T.X, T.Y, T.Z);
			fprintf(s_pf, "T: x:%d, y:%d, z:%d\n", FMath::FloorToInt32(T.X), FMath::FloorToInt32(T.Y), FMath::FloorToInt32(T.Z));

			//for (int32  idx = 0; idx<vec.Num(); ++idx)
			//{
			//	auto& it = vec[idx];
			//	const auto& T = it.GetTranslation();
			//	const auto& Q = it.GetRotation();
			//	const auto& S = it.GetScale3D();
			//	auto boneName = skelMesh->GetBoneName(idx);
			//	fprintf(s_pf, "Index: %d, Name: %s\n", idx, TCHAR_TO_ANSI(*boneName.ToString()));
			//	fprintf(s_pf, "T: x:%.6f, y:%.6f, z:%.6f\n", T.X, T.Y, T.Z);
			//	fprintf(s_pf, "Q: x:%.6f, y:%.6f, z:%.6f, w:%.6f\n", Q.X, Q.Y, Q.Z, Q.W);
			//	fprintf(s_pf, "S: x:%.6f, y:%.6f, z:%.6f\n", S.X, S.Y, S.Z);
			//}
		}
	}
}
...
static void TestBegin(UActorComponent* Target)
{
	...
	const uint32 testEndFrame = testTotalFramesCount;//测试结束和清理
	const uint32 testFinishFrame = testEndFrame - 1;//测试完成
	float testDeltaTime = 2.0f / testFramesCount;//seconds / frames, 其中1.0f为测试帧总时长, 如停步过程的时长(胶囊体开始停到不动)
	const float testStartFrameDeltaTime = 0.0f;//开始测试的时间, 如从跑开始停步; 10.0f表示一个足够大的时间用于跳过起跑, 0.9f为微调时间, 10.0f + 0.9f时刻姿势差不多为跑状态动作左脚与地面平行即将着地
	auto testFirstComponent = Cast<UCharacterMovementComponent>(Target);
	const bool isTestLoopBegin = testFirstComponent != nullptr;//按component的遍历顺序的首个component, isTestLoopBegin为true即一个测试循环开始, 如可用于逐帧调试设置断点
	...
	if (auto movement = testFirstComponent)
	{
		const auto testMiddleFrame = testFramesCount / 2;
		auto testFrame = g_testFrame;
		if (g_testFrame == testStartFrame)
		{
			s_pf = fopen("E:/Pose_UE.txt", "w");
		}
		if (g_testFrame < testMiddleFrame)
		{
			movement->AddInputVector(FVector(1.0f, 0.0f, 0.0f));
		}
		if (g_testFrame == testFinishFrame)
		{
			auto TM = owner->GetActorTransform();
			TM.SetTranslation(FVector(0.0f, 0.0f, 100.0f));
			owner->SetActorTransform(TM);
...
void FActorComponentTickFunction::ExecuteTick(float DeltaTime, enum ELevelTick TickType, ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
{
	...
	ExecuteTickHelper(Target, Target->bTickInEditor, DeltaTime, TickType, [this, TickType](float DilatedTime)
	{
		...
		{
			//在Character Blueprint中添加名为FixedDeltaTimeBasedTest的变量即可启用固定deltaTime测试
			//本例将每帧固定deltaTime测试在Tick中调用, 并确保Tick和渲染结果同步, 以便观察.
			//如有需要, 如不关心渲染结果, 也可在一个循环中执行全部测试帧, 或通过某个按钮, 全局变量开关(CVar)执行测试
			if (TestHasProperty(owner, TEXT("FixedDeltaTimeBasedTest")))
			{
				TestBegin(Target);
				Target->TickComponent(g_testDeltaTime, TickType, this);
				TestEnd(Target);
				return;
			}
		}
